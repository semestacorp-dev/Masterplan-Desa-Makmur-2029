...
import { 
  ...
  TrendingUp, Activity, Home, BookOpen, MapPin, Wheat, Factory, Palette, GraduationCap
} from 'lucide-react';
...
export const ID6RadarDiagram: React.FC = () => {
  ...
  const data = [
    { subject: 'Layanan Dasar', A: 120, B: 110, fullMark: 150, desc: 'Kesehatan, Pendidikan, Air Bersih' },
    { subject: 'Sosial', A: 98, B: 130, fullMark: 150, desc: 'Modal Sosial, Keamanan, Toleransi' },
    { subject: 'Ekonomi', A: 86, B: 130, fullMark: 150, desc: 'Akses Kerja, Pasar, Lembaga Ekonomi' },
    { subject: 'Lingkungan', A: 99, B: 100, fullMark: 150, desc: 'Kualitas Lingkungan, Bencana' },
    { subject: 'Aksesibilitas', A: 85, B: 90, fullMark: 150, desc: 'Jalan, Transportasi, Internet' },
    { subject: 'Tata Kelola', A: 65, B: 85, fullMark: 150, desc: 'Transparansi, Partisipasi, Akuntabilitas' },
  ];
      ...
      <div className="w-full md:w-1/2 h-80 relative bg-white/10 rounded-2xl backdrop-blur-sm border-2 border-white/20 p-4">
         <ResponsiveContainer width="100%" height="100%">
            <RadarChart cx="50%" cy="50%" outerRadius="70%" data={data}>
              ...
              <PolarAngleAxis dataKey="subject" tick={{ fill: '#C6FF00', fontSize: 12, fontWeight: '900', fontFamily: 'Righteous' }} />
        ...
        <div className="grid grid-cols-2 gap-3">
          {data.map((item, idx) => (
             <motion.div 
                ...
                className={`p-3 rounded-lg border-2 transition-all cursor-default ${activeDim === idx ? 'bg-pop-primary text-pop-dark border-pop-dark shadow-pop' : 'bg-white/10 border-white/20 text-stone-300 hover:bg-white hover:text-pop-dark hover:border-pop-dark'}`}
             >
                <div className="text-xs font-black uppercase tracking-wider mb-1">{item.subject}</div>
                <div className={`text-[10px] leading-tight ${activeDim === idx ? 'text-pop-dark' : 'text-stone-400'}`}>{item.desc}</div>
             </motion.div>
          ))}
        </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row h-auto lg:h-[500px]">
            ...
            <div className="w-full lg:w-2/3 p-8 lg:p-12 flex flex-col justify-center bg-pop-light relative z-0">
                ...
                <AnimatePresence mode='wait'>
                    ...
                    >
                        <div className={`inline-block px-3 py-1 rounded-md text-xs font-black tracking-widest uppercase mb-4 border-2 border-pop-dark shadow-pop-sm ${missions[activeMission].color}`}>
                             Program Prioritas
                        </div>
                        <h2 className="font-display text-5xl lg:text-6xl text-pop-dark mb-6 leading-tight">
                            {missions[activeMission].program}
                        </h2>
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            <div className="bg-white p-6 rounded-xl border-2 border-pop-dark shadow-pop">
                                ...
                                <div className={`text-xl font-display ${missions[activeMission].color.includes('bg-pop') ? 'text-pop-dark' : 'text-pop-primary'}`}>
                                    {missions[activeMission].focus}
                                </div>
                            </div>
...
export const PilotVillagesGrid: React.FC = () => {
    const villages = [
        { 
            ...
            niche: "Industri Mocaf", 
            status: "MAJU",
            desc: "Sentra produksi singkong.",
            color: "bg-pop-primary"
        },
        { 
            name: "Banjar Rejo", 
